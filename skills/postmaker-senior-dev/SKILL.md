...
### 5. Mandatory Deploy Workflow
`bash scripts/ship.sh` is the only authorized way to push code to `staging` or `main`. Never run manual `git push` directly to deployment branches.
