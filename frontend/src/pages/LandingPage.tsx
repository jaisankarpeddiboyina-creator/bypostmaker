    ...
    <div className="landing">
      ...
      <section className="hero">
        <div className="hero-inner-grid">
          ...
          {/* Interactive Live PostMaker Studio Preview Card */}
          <div className="hero-interactive-card glass-card" id="demo">
            <div className="studio-card-header">
              <div className="studio-header-title">
                <Sparkles size={15} className="text-cyan" />
                <span>PostMaker Studio — Live Interactive Demo</span>
              </div>
              <div className="studio-badge-pill">30+ Platforms Ready</div>
            </div>
            ...
            {/* Prompt Box Preview */}
            <div className="studio-input-preview">
              <div className="input-preview-label">Your Prompt Input:</div>
              <div className="input-preview-text">&ldquo;{activePreset.prompt}&rdquo;</div>
            </div>

            {/* Platform Selection Tabs */}
            <div className="studio-tabs-row">
              {(['linkedin', 'twitter', 'instagram', 'reddit', 'tiktok', 'youtube'] as PlatformTab[]).map((tab) => (
                <button
                  key={tab}
                  className={`studio-tab ${activeTab === tab ? 'active' : ''}`}
                  onClick={() => setActiveTab(tab)}
                >
                  <PlatformIcon id={tab} size={15} useBrandColor={activeTab === tab} />
                  <span>{tab === 'twitter' ? 'X / Twitter' : tab.charAt(0).toUpperCase() + tab.slice(1)}</span>
                </button>
              ))}
            </div>

            {/* Dynamic Native Output Preview */}
            <div className="studio-output-box">
              <div className="output-box-header">
                <div className="output-platform-badge">
                  <PlatformIcon id={activeTab} size={16} useBrandColor />
                  <span>Formatted for {activeTab === 'twitter' ? 'X / Twitter' : activeTab.toUpperCase()}</span>
                </div>
                <div className="output-box-actions">
                  <span className="char-count">
                    {activePreset.outputs[activeTab].length} chars
                  </span>
                  <button className="copy-demo-btn" onClick={handleCopyDemo}>
              ...
              <div className="output-content-text">
                {activePreset.outputs[activeTab]}
              </div>
            </div>
            ...
            <div className="studio-card-footer">
              <div className="studio-footer-note">
                ...
                <span>Packaged into ZIP folder structure per platform</span>
              </div>
              <button className="btn btn-sm btn-primary" onClick={handleCTA}>
                <Download size={14} />
                <span>Try Studio Free</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <style>{`
        ...
        /* Interactive Demo Studio Card */
        .hero-interactive-card {
          padding: 24px;
          border-radius: 24px;
          background: rgba(14, 27, 46, 0.82);
          border: 1px solid rgba(255, 255, 255, 0.25);
          box-shadow: 0 32px 80px -16px rgba(2, 132, 199, 0.40);
        }
        .studio-card-header {
          display: flex;
          align-items: center;
        ...
        .text-cyan { color: var(--color-primary-start); }
        ...
        .copy-demo-btn:hover { background: rgba(56, 189, 248, 0.2); }
        .output-content-text {
          font-size: 13.5px;
          line-height: 1.6;
          color: var(--color-text-primary);
