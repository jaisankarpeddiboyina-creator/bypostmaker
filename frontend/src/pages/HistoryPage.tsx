...
interface HistoryCampaign {
  ...
  created_at: number
  posts: Array<{ platform_id: string; content: string; edited: number }>
}

export default function HistoryPage() {
  const { addToast, setPrompt, setSelectedPlatforms, setCampaign } = useAppStore()
  ...
  const load = async (page = 1) => {
    ...
    try {
      ...
      setPagination({ page, total: res.pagination.total, pages: res.pagination.pages })
    } catch {
  ...
  const handleCopyPost = async (content: string, key: string) => {
    await navigator.clipboard.writeText(content)
    setCopiedKey(key)
    setTimeout(() => setCopiedKey(null), 2000)
  }

  // Re-use a campaign — pre-fill the app with the same prompt and platforms
  const handleReuse = (campaign: HistoryCampaign) => {
    setPrompt(campaign.prompt)
    setSelectedPlatforms(campaign.platforms)
    // Note: images/videos are not stored — user will need to re-upload if needed
    addToast('Prompt loaded. Re-upload any media if needed.', 'info')
    navigate('/app')
  }
    ...
    <div className="history-page">
      <div className="history-inner">
        <div className="history-header">
          ...
          <p className="history-sub">{pagination.total} campaigns</p>
        </div>
          ...
          <div key={c.id} className="history-card">
            ...
            >
              <div className="history-card-meta">
                <span className="history-card-date">{formatDate(c.created_at)}</span>
                <span className="history-card-count">{c.generated_count} platforms</span>
                {c.has_video ? <span className="history-tag">Video</span> : null}
                {c.has_image ? <span className="history-tag">Image</span> : null}
              </div>

              <p className="history-card-prompt">{c.prompt}</p>

              <div className="history-card-platforms">
                {c.platforms.slice(0, 8).map(id => (
                  <span key={id} className="history-platform-pill">
                    {PLATFORM_MAP[id]?.name ?? id}
                  </span>
                ))}
                {c.platforms.length > 8 && (
                  <span className="history-platform-more">+{c.platforms.length - 8} more</span>
                )}
              </div>
              ...
              <div className="history-card-actions" onClick={e => e.stopPropagation()}>
                <button
                  ...
                  title="Re-use this prompt and platforms"
                >
                  <RefreshCw size={13} />
                  Re-use prompt
                </button>
                  ...
                  <div key={post.platform_id} className="history-post">
                    <div className="history-post-header">
                      <span className="history-post-platform">
                        {PLATFORM_MAP[post.platform_id]?.name ?? post.platform_id}
                      </span>
                      {post.edited ? <span className="history-post-edited">edited</span> : null}
                      <button
                      ...
                      >
                        {copiedKey === `${c.id}:${post.platform_id}`
                          ? <Check size={13} />
                          : <Copy size={13} />
                        }
                      </button>
      ...
      <style>{`
        ...
        .history-sub { font-size: 13px; color: var(--text-3); margin-top: 4px; }

        .history-loading { display: flex; flex-direction: column; gap: 12px; }
        .history-skeleton {
          height: 80px;
          background: var(--card);
        ...
        .history-reuse-btn { height: 30px; font-size: 12px; }
        ...
        .history-post:last-child { border-bottom: none; }
        .history-post-header {
          display: flex;
          align-items: center;
          gap: 8px;
        ...
        .history-copy-btn { margin-left: auto; }
